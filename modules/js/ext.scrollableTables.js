(function () {
    ...
    function headerListener() {
        ...
        for (var i=d.length-1;i>=0;i--) {
            ...
            // look at each cell in the body first row, and make the corresponding header cell have the same width
            for (var j=0;j<cells.length;j++) {
                var c = cells[j];
                // first we get the computed width of the cell
                var s = window.getComputedStyle(c).width;
                // the actual width includes padding, so first we remove that
                var p = addpx(window.getComputedStyle(c).paddingLeft,window.getComputedStyle(c).paddingRight);
                var w = subpx(s,p);
                // sortable tables have extra padding on the right for the sort symbol in the header.  So we
                // leave room for that too.
                var xp = subpx(window.getComputedStyle(row.cells[j]).paddingRight,window.getComputedStyle(row.cells[j]).paddingLeft);
                w = subpx(w,xp);
